...
# 1. Criar o arquivo CSV localmente com os dados que você forneceu
# Estou salvando como 'drivers.csv' no diretório atual
csv_content = r"""driverId,driverRef,number,code,forename,surname,dob,nationality,url
...
"""
# Nota: Coloquei uma amostra acima para o código não ficar gigante. 
# Se você tiver o arquivo completo, certifique-se de que ele está na mesma pasta.

with open("drivers.csv", "w", encoding="utf-8") as f:
    f.write(csv_content)

print("Arquivo 'drivers.csv' criado com sucesso.")

# ==========================================
# CÉLULA 3: Início do PySpark
# ==========================================

# Inicializar a Sessão Spark (Local)
# .master("local[*]") diz para usar todos os núcleos da sua CPU
spark = SparkSession.builder \
    .appName("F1 Analysis Local") \
    .master("local[*]") \
    .getOrCreate()
...
# 1. Carregar Dados
# inferSchema=True tenta adivinhar que datas são datas e números são números
df = spark.read.csv("drivers.csv", header=True, inferSchema=True, nullValue="\\N")

# 2. Transformação de Dados (Calculando Idade)
# Selecionamos apenas quem tem data de nascimento válida
df_clean = df.select("forename", "surname", "dob", "nationality").dropna()

# Calculamos a idade em anos
df_features = df_clean.withColumn("age", floor(datediff(current_date(), col("dob")) / 365.25))

print("\n--- Amostra dos Pilotos e Idades Calculadas ---")
df_features.show(5)

# ==========================================
# CÉLULA 4: Machine Learning (K-Means)
# ==========================================
print("--- Executando Clusterização (K-Means) ---")

# O Spark ML precisa que as features de entrada sejam uma coluna de Vetores
assembler = VectorAssembler(inputCols=["age"], outputCol="features")
df_vector = assembler.transform(df_features)

# Criar o modelo K-Means para separar em 3 grupos (ex: Jovens Talentos, Auge, Veteranos)
kmeans = KMeans().setK(3).setSeed(1).setFeaturesCol("features")
model = kmeans.fit(df_vector)

# Fazer as predições (atribuir cada piloto a um grupo 0, 1 ou 2)
predictions = model.transform(df_vector)

# Mostrar os centróides (a idade média de cada grupo)
centers = model.clusterCenters()
print("Centróides dos Grupos (Idade Média Aproximada):")
for i, center in enumerate(centers):
    print(f"Grupo {i}: {center[0]:.1f} anos")

# Mostrar contagem por grupo
print("\nQuantidade de pilotos por grupo:")
predictions.groupBy("prediction").count().show()

# ==========================================
# CÉLULA 5: Visualização (Matplotlib)
# ==========================================
# Convertemos para Pandas apenas para o gráfico (Spark é ruim de gráfico nativo)
pdf = predictions.select("age", "prediction").toPandas()
...
# Plotar histograma
plt.hist(pdf['age'], bins=15, color='skyblue', edgecolor='black', alpha=0.7, label='Todos')

# Adicionar linhas verticais para as médias dos grupos
for i, center in enumerate(centers):
    plt.axvline(center[0], color=colors.get(i, 'black'), linestyle='dashed', linewidth=2, label=f'Média Grupo {i}')

plt.title('Distribuição de Idade dos Pilotos e Grupos K-Means')
plt.xlabel('Idade')
