...
# ==========================================
# CÉLULA 2: Carregamento dos Dados e Criação do Grafo
# Rubrica: Carregamento e Preparação (2 pontos)
# ==========================================

# Dados CSV (Apenas os 10 primeiros para teste, você pode colar o resto depois)
csv_data = r"""driverId,driverRef,number,code,forename,surname,dob,nationality,url
...
806,"oscar_gonzalez",\N,\N,"Óscar","González","1923-11-10","Uruguayan","http://en.wikipedia.org/wiki/%C3%93scar_Gonz%C3%A1lez_(racing_driver)"
...
# Adicionar nós (Pilotos)
# Usaremos o 'driverId' como ID do nó e guardaremos o sobrenome como atributo para exibir depois
for index, row in df.iterrows():
    G.add_node(row['driverId'], label=f"{row['forename']} {row['surname']}", nationality=row['nationality'])

# CRIAR ARESTAS (Regra: Conectar pilotos da mesma nacionalidade)
# Se A e B são do mesmo país, criamos um link A -> B e B -> A
paises = df['nationality'].unique()

print("Gerando conexões baseadas em nacionalidade...")
for pais in paises:
    # Pegar lista de IDs dos pilotos desse país
    pilotos_pais = df[df['nationality'] == pais]['driverId'].tolist()
    
    # Se houver mais de 1 piloto do mesmo país, conectar todos entre si
    if len(pilotos_pais) > 1:
        for p1 in pilotos_pais:
            for p2 in pilotos_pais:
                if p1 != p2: # Não criar link de si para si mesmo
                    G.add_edge(p1, p2)

print(f"Grafo carregado: {G.number_of_nodes()} nós e {G.number_of_edges()} arestas.")

# ==========================================
# CÉLULA 3: Implementação do PageRank do Zero
# Rubrica: Implementação do Algoritmo (3 pontos)
# ==========================================

def manual_pagerank(graph, d=0.85, tol=1.0e-6, max_iter=100):
    """
    Implementação manual do algoritmo PageRank.
    """
    ...
    # Nós sem saída (Dangling nodes)
    dangling_nodes = [n for n in graph if graph.out_degree(n) == 0]
    
    for i in range(max_iter):
        prev_pagerank = pagerank.copy()
        
        # Soma da massa dos dangling nodes
        dangling_sum = sum(prev_pagerank[node] for node in dangling_nodes)
        
        # Parte constante da fórmula (Teletransporte + Dangling)
        base_value = (1.0 - d) / N + (d * dangling_sum) / N
        
        diff = 0
        
        for node in graph.nodes():
            incoming_sum = 0
            # Iterar sobre quem aponta para o nó atual
            for source in graph.predecessors(node):
                incoming_sum += prev_pagerank[source] / graph.out_degree(source)
            
            pagerank[node] = base_value + (d * incoming_sum)
...
# Executar a implementação manual
print("Calculando PageRank Manual...")
pr_manual = manual_pagerank(G, d=0.85)

# ==========================================
# CÉLULA 4: Comparação com NetworkX e Exibição
# Rubrica: Análise e Comparação
# ==========================================

print("Calculando PageRank NetworkX...")
pr_nx = nx.pagerank(G, alpha=0.85, tol=1.0e-6)

# Função para formatar a exibição
def mostrar_ranking(pr_dict, graph, top_k=10):
    sorted_pr = sorted(pr_dict.items(), key=lambda x: x[1], reverse=True)[:top_k]
    print(f"{'Rank':<5} | {'Score':<10} | {'Nome do Piloto':<25} | {'Nacionalidade'}")
    print("-" * 65)
    for i, (node_id, score) in enumerate(sorted_pr):
        nome = graph.nodes[node_id]['label']
        nac = graph.nodes[node_id]['nationality']
        print(f"{i+1:<5} | {score:.6f}   | {nome:<25} | {nac}")

print("\n--- Resultado PageRank (Top 10) ---")
mostrar_ranking(pr_manual, G, 10)

# ==========================================
# CÉLULA 5: Análise dos Resultados
# ==========================================
print("\n--- Análise Rápida ---")
print("Neste exemplo reduzido com apenas 10 pilotos, os pilotos alemães (German)")
print("devem ter o maior PageRank, pois existem 3 deles (Heidfeld, Rosberg, Glock)")
print("conectados entre si. Os finlandeses (Kovalainen, Raikkonen) vêm em seguida")
print("pois têm 2 conexões. Os outros aparecem isolados com score menor.")

# Visualização do Grafo
plt.figure(figsize=(10, 8))
pos = nx.spring_layout(G, seed=42)
# Tamanho do nó baseado no PageRank
node_sizes = [pr_manual[n] * 5000 for n in G.nodes()]
nx.draw(G, pos, with_labels=False, node_size=node_sizes, node_color='skyblue', edge_color='gray', arrows=True)
# Adicionar rótulos dos nomes
labels = nx.get_node_attributes(G, 'label')
nx.draw_networkx_labels(G, pos, labels, font_size=8)
plt.title("Rede de Pilotos (Conectados por Nacionalidade)")
plt.show()